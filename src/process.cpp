...
#include <iostream> // Biblioth�que standard pour l'utilisation des flux d'entree, sortie qu'offre la bibliotheque standard
#include <string> // Manipulation chaine de caractere de classe std::string
#include <list> // Manipulation de tableau liste doublement chainee  
#include <cstdlib> // Biblioth�que standard pour l'utilisation de fonction utilitaire en C++
...
#include <tchar.h>
#include <TlHelp32.h>
// Biblioth�que statique personnelle 
...
Process::Process(std::string windowName)
{
	...
	DWORD cbNeeded; // nombre de d'octet n�c�ssaire pour stocker le module 
	...
	else
	{
		...
		GetModuleBaseName(hProcess, moduleHandle, pName, sizeof(pName) / sizeof(TCHAR)); // Recupere le nom en fonction du handle process et module 
																						 ...
																						 // Stocker les donnees dans la class process
...
DWORD Process::attachModule(std::string moduleName)
{
	...
	HANDLE hModule = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, pId); //Prend un instantane des processus specifies, ainsi que des tas, modules et threads utilises par ces processus.
	MODULEENTRY32 mEntry; // Liste d'entree de module appartenant au processus
